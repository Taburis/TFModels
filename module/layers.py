...
class shortCut2D(tf.keras.layers.Layer):
    ...
    def __init__(self, 
        ...
        super(shortCut2D, self).__init__(self, **kwargs)
...
#   print('filters for shortcut: ', nfilter, shortcut.shape[dim])
    ...
    def get_config(self):
        return {'use_projection':self.use_projection, 'training':self.training}

class residual2D(tf.keras.layers.Layer):
    def __init__(self, layers, training = None , **kwargs):
        super(residual2D, self).__init__(self, training = training, **kwargs)
        self.layers = layers
        self.shortcut = shortCut2D(training = traning)
    ...
    def get_config(self):
        return {'layers':self.layers, 'training':self.training}
...
class batch_norm_activation(tf.keras.layers.Layer):
    def __init__(self,
        ...
        Args:
          ...
          training: `bool`, if True also add variables to the graph collection
        ...
        """
        super(batch_norm_activation, self).__init__(training=training, **kwarg)
        if init_zero:
