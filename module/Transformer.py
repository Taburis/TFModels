...
def sinusoid_position_encoder_generator(
    ...
    def get_frequency(pos, i, d_model):
        return np.float32(pos)/np.power(frequency_base, (2*(i//2))/np.float32(d_model))
    def position_encoding_fn(position, d_model):
        angle_rads = get_frequency(np.arange(position)[:, np.newaxis],
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, 
                   maximum_position_encoding, positional_encoding, dropout_rate=0.1):
...
class Transformer(tf.keras.layers.Layer):
    def __init__(self, num_layers = 6, d_model = 512, num_heads = 8, dff = 2048, 
        ...
        self.freq_length = pe_input
        if pe_input < pe_target: self.freq_length = pe_target

        self.positional_encoder = sinusoid_position_encoder_generator(self.freq_length)
        ...
        self.tokenizer = Encoder(num_layers, d_model, num_heads, dff,
                                 pe_input, self.positional_encoder, dropout_rate)
        
        self.decoder = Decoder(num_layers, d_model, num_heads, dff,
                               pe_target, self.positional_encoder, dropout_rate)
    ...
    def call(self, inp, tar, training, enc_padding_mask,
        ...
        enc_output = self.tokenizer(x, training, enc_padding_mask)  # (batch_size, inp_seq_len, d_model)
        
        # dec_output.shape == (batch_size, tar_seq_len, d_model)
        tar = self.decoder_embedding(x)
